...
ip="$(kubectl get ingress smartcity-streamlit-public \
  -n "${namespace}" \
  -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || true)"

if [[ -n "${domain}" ]]; then
  echo "Public demo URL: https://${domain}"
  if [[ -n "${ip}" ]]; then
...
if [[ -z "${ip}" ]]; then
  echo "Public demo URL is not ready yet; the Ingress has no load balancer IP." >&2
  exit 1
fi

echo "Public demo URL: http://${ip}"
